...
export function SessionPage({ session, onExit }: SessionPageProps) {
  ...
  // Initialiser et maintenir l'état du sidebar selon le device
  useEffect(() => {
          ...
          <div className="flex-1 flex flex-col lg:flex-row gap-6 sm:gap-8 items-center lg:items-start min-w-0">
            <div className="flex-1 flex flex-col items-center min-w-0">
              ...
              <div className="mt-6 sm:mt-8 w-full flex flex-col items-center">
                <BoardControls
                  onStart={() => session.goToMove(0)}
                  onPrev={session.goToPreviousMove}
                  onNext={session.goToNextMove}
                  onEnd={() => session.goToMove(currentLine?.moves.length || 0)}
                  canPrev={state.currentMoveIndex > 0}
                  canNext={currentLine ? state.currentMoveIndex < currentLine.moves.length : false}
                  showThreats={showThreats}
                  onToggleThreats={() => setShowThreats((v) => !v)}
                  currentInfo={
                    currentLine 
                      ? `${state.currentMoveIndex > 0 ? currentLine.moves[state.currentMoveIndex - 1].san : "Départ"}` 
                      : ""
                  }
                />
                
                <div className="mt-6 flex items-center gap-3 bg-black text-white px-4 py-2 rounded-full border-[3px] border-black shadow-[4px_4px_0_0_#ccc]">
                  <Gamepad2 size={16} className="text-wero-cyan" />
...
function CourseSidebar({
  ...
  return (
    <aside className="w-full h-[calc(100dvh-5rem)] max-h-[calc(100dvh-5rem)] lg:w-[420px] xl:w-[460px] shrink-0 lg:sticky lg:top-6 lg:h-[calc(100dvh-3rem)] lg:max-h-[calc(100dvh-3rem)]">
      <div className="wero-card custom-scrollbar bg-white overflow-y-auto overscroll-contain touch-pan-y h-full min-h-0 flex flex-col">
        <div className="sticky top-0 z-10 p-4 border-b-[3px] border-black bg-wero-yellow flex items-center justify-between gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p
                      className={`text-[10px] font-bold ${
                        isOpen ? "text-white/60" : "text-gray-50"
                      }`}
                    >
